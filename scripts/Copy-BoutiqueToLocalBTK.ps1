...
<#
.SYNOPSIS
Copie table par table la base SQL Server distante BOUTIQUE vers la base locale BTK.
...
.EXAMPLE
powershell -ExecutionPolicy Bypass -File .\scripts\Copy-BoutiqueToLocalBTK.ps1 -SaveRemoteCredential -Force

.EXAMPLE
powershell -ExecutionPolicy Bypass -File .\scripts\Copy-BoutiqueToLocalBTK.ps1 -SetRemoteCredentialOnly

.EXAMPLE
powershell -ExecutionPolicy Bypass -File .\scripts\Copy-BoutiqueToLocalBTK.ps1 -RegisterDailyTask -SaveRemoteCredential

.EXAMPLE
powershell -ExecutionPolicy Bypass -File .\scripts\Copy-BoutiqueToLocalBTK.ps1 -LocalServer "localhost\SQL2022" -UseLocalSqlLogin -LocalUser "sa" -SaveLocalCredential -SaveRemoteCredential -Force
#>

[CmdletBinding()]
param(
    [string]$RemoteServer = "mssql-177328-0.cloudclusters.net,18562",
    [string]$RemoteDatabase = "BOUTIQUE",
    [string]$RemoteUser = "guyak89",

    [string]$LocalServer = "localhost\SQL2022",
    [string]$LocalDatabase = "BTK",
    [switch]$UseLocalSqlLogin,
    [string]$LocalUser = "sa",

    [string]$RemoteCredentialPath = (Join-Path $env:APPDATA "SqlTableCopy\BOUTIQUE_remote.credential.xml"),
    [string]$LocalCredentialPath = (Join-Path $env:APPDATA "SqlTableCopy\BTK_local.credential.xml"),
    ...
    [string]$TaskName = "Copie SQL BOUTIQUE vers BTK",
    [switch]$RunNow,
...
$script:LogPath = Join-Path $LogDirectory ("BOUTIQUE_to_BTK_{0}.log" -f (Get-Date -Format "yyyyMMdd_HHmmss"))
$script:ReportPath = Join-Path $ReportDirectory ("BOUTIQUE_to_BTK_{0}.md" -f (Get-Date -Format "yyyyMMdd_HHmmss"))
...
function Write-CopyReport {
    ...
    $lines.Add("# Rapport copie SQL BOUTIQUE vers BTK")
    $lines.Add("")
...
function New-SqlConnection {
    ...
    $builder["Application Name"] = "Copy-BoutiqueToLocalBTK"
...
function Register-BoutiqueDailyTask {
    param(
    ...
    Register-ScheduledTask -TaskName $TaskName -Action $action -Trigger $trigger -Principal $principal -Settings $settings -Description "Copie quotidienne SQL Server BOUTIQUE vers la base locale BTK." -Force | Out-Null
...
try {
    ...
    if ($RegisterDailyTask) {
        Register-BoutiqueDailyTask -ScriptPath $PSCommandPath
        if (-not $RunNow) {
            return
        }
    }
