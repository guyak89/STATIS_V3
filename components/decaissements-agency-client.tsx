...
import { exportDecaissementsAgencyToExcel } from "@/lib/excel-export";
...
const fetcher = async (url: string): Promise<DecaissementsPayload> => {
            ...
            {rows.map((row, index) => {
              ...
              return (
                <tr key={`${kind}-row-${row.code}`}>
                  ...
                  <td>
                    <Link
                      href={detailHref(`/detail/decaissements/${encodeURIComponent(agencyCode)}/${kind === "product" ? "produit" : "gestionnaire"}/${encodeURIComponent(row.code)}`)}
                      className="detail-drill-link"
                      title={`Voir les dossiers decaisses pour ${row.name}`}
                    >
                      <span className="collector-name">
                        <strong>{row.name}</strong>
                        <span>Code {row.code}</span>
                      </span>
                      <span className="detail-drill-sub">Liste nominative des dossiers</span>
                    </Link>
                  </td>
                  <td>
...
export function DecaissementsAgencyClient({ agencyCode }: { agencyCode: string }) {
  ...
  const { data, error, isLoading } = useSWR(
    detailHref(`/api/detail/decaissements/${encodeURIComponent(normalizedAgencyCode)}`),
    fetcher,
  ...
  const header = (
    <header className="app-header">
      <Link href={detailHref("/detail/decaissements")} className="back-btn" title="Retour au detail par agence">
        Retour
      </Link>
      ...
      <div className="header-brand">
        ...
        <div>
          <div className="brand-name">Decaissements par Agence</div>
      ...
      {data && (
        <div className="header-actions" style={{ marginLeft: "auto" }}>
          <button
            ...
            onClick={() =>
              exportDecaissementsAgencyToExcel({
        ...
        <div className="app-content">
          <div className="panel">
            <div className="error-panel">
              ...
              <div className="error-title">Erreur lors du chargement du detail decaissement</div>
  ...
  return (
    <div className="app">
      ...
      <div className="app-content fade-in">
        <div className="detail-summary-row">
          ...
          <div className="detail-summary-card" style={{ borderTopColor: "var(--teal)" }}>
            ...
            <div className="detail-summary-sub">
              {fmtInt.format(data.operations)} operation(s) de decaissement
            </div>
          </div>
          ...
          <div className="detail-summary-card" style={{ borderTopColor: "var(--green)" }}>
            ...
            <div className="detail-summary-sub">
              {topProduct ? topProduct.name : "Aucun decaissement"}
            </div>
          </div>
        ...
        <DispatchSection
          title={`${data.agencyCode} - Decaissements par produit de credit`}
          kicker="Produit"
          rows={data.productRows}
          total={data.total}
          kind="product"
          emptyLabel="Aucun decaissement par produit sur la periode."
          agencyCode={data.agencyCode}
          detailHref={detailHref}
        />

        <DispatchSection
          title={`${data.agencyCode} - Decaissements par gestionnaire`}
          kicker="Gestionnaire"
          rows={data.managerRows}
          total={data.total}
          kind="manager"
          emptyLabel="Aucun decaissement par gestionnaire sur la periode."
          agencyCode={data.agencyCode}
          detailHref={detailHref}
        />
      </div>
      ...
      <footer className="app-footer">
        <Link href={detailHref("/detail/decaissements")} style={{ color: "var(--teal)", textDecoration: "none" }}>
          Retour au detail decaissements
        </Link>
        <span className="footer-dot" />
        <span>BASE_INTERCO - Detail decaissements {data.agencyCode}</span>
        <span className="footer-dot" />
        <span>Source : <code>DECAIS</code> / <code>DEMPRET</code> / <code>PRDT_CRD</code> / <code>GESTIONNAIRE</code></span>
      </footer>
    </div>
  );
}
